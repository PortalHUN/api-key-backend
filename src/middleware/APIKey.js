...
const apiKey = async (req, res, next) => {
  ...
  try {
    ...
    await db.query(
      `SELECT * FROM api_keys WHERE AppName = ${db.escape(obj.AppName)};`,
      (error, result) => {
        try {
          if (error) throw "Database error msg: " + error;
            ...
            !(
              result[0].AppName == obj.AppName &&
              result[0].ApiKey == obj.ApiKey &&
              result[0].Blocked == 0
            )
          )
    ...
    return res
      .status(500)
      .json({ err: error.message || error ? "Invalid JWT Token" : error });
  }
};
