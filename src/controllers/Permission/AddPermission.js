...
const AddPermission = async (req, res) => {
  ...
  await db.query(`SELECT Perm FROM permissions WHERE AppID = ${db.escape(ID)};`, async (error, perm) => {
    if (error) return res.status(500).json({ err: "Database error: " + error.sqlMessage });
    let permission = [];
    perm.forEach((element) => {
      permission.push(element.Perm);
    });

    let correct = [];
    permissions.forEach(async (element) => {
      if (PermissionList.includes(element) && !permission.includes(element)) correct.push(element);
    });
    ...
    await db.query(`SELECT ID FROM api_keys WHERE ID = ${db.escape(ID)};`, async (error, app) => {
      if (error) return res.status(500).json({ err: "Database error: " + error.sqlMessage });
      else if (!app || app.length == 0) return res.status(400).json({ err: "No App found with this ID" });
